...
template <typename Matrix_type> auto trace(Matrix_type &A) {
  ...
  if (M != N) {
    throw std::runtime_error(
        "ERROR IN TRACE: Rows and columns must be of equal size.");
  }
  ...
  A.dev()
      ...
      .submit([&](sycl::handler &h) {
        const auto reduction_func =
            sycl::reduction(trace_buf, h, sycl::plus<Scalar_T>());
