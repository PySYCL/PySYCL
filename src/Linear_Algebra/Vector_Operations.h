...
auto vector_addition(Tensor_T& A, Tensor_T& B) {
  if(A.len() != B.len() || A.num_dims() != B.num_dims()) {
    throw std::runtime_error("ERROR in Vector Dot Product: Tensors of incompatible dimensions!");
  }
  ...
  device.get_queue().submit([&](sycl::handler& h) {
    ...
    h.parallel_for<class vector_addition_kernel>(sycl::range<1>(N), [=](sycl::id<1> idx) {
